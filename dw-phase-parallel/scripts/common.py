...
def load_design(root: Path) -> list[dict]:
    ...
    for number in range(1, count + 1):
        ...
        if any(
            not item.startswith("phase") or not item[5:].isdigit()
        ...
        phases.append(
            {
                ...
                "phase_type": metadata(body, "Phase Type") or "",
...
def init(root: Path, mode: str, max_parallel: int) -> dict:
    ...
    if (
        not current.exists()
        or metadata(current.read_text(encoding="utf-8"), "Status") != "REVIEWED"
        or metadata(current.read_text(encoding="utf-8"), "Global Step") != "1"
    ):
...
def ready_phases(state: dict) -> list[dict]:
    active = {
        ...
        if phase["status"]
        in {"RUNNING", "REVIEW_PENDING", "READY_TO_COMMIT", "PR_OPEN"}
    }
    result = []
    for phase in state["phases"].values():
        if phase["status"] != "READY":
            continue
        if all(
            state["phases"][dep]["status"] in {"MERGED", "COMPLETED"}
...
def phase_slug(phase: dict) -> str:
    ...
    return (
        re.sub(r"[^a-z0-9]+", "-", phase["name"].lower()).strip("-")[:48].strip("-")
        or phase["id"]
    )
...
def context_document(root: Path, relative_path: str) -> str:
...
def prompt(root: Path, state: dict, phase: dict, light: bool) -> str:
    ...
    instructions = (
        ...
        if light
        else f"現在は step {phase['step']} です。指定された成果物だけを作成し、後続 step を変更しないでください。完了後、対象テストを実行してください。"
    )
    return f"""あなたは phase 実装 agent です。
...
def spawn(root: Path, state: dict, phase: dict, args: argparse.Namespace) -> None:
    ...
    if args.light and "codex exec" in command:
        raise RuntimeError(
            "light mode は codex exec を起動しません。native subagent を使用してください"
        )
    ...
    if command == "native-subagent":
        request_file = (
            root / STATE_DIR / "requests" / f"{phase['id']}-{int(time.time())}.md"
        )
        request_file.parent.mkdir(parents=True, exist_ok=True)
        request_file.write_text(
            prompt(root, state, phase, args.light)
        ...
        print(
            f"{phase['id']}: native subagent request -> {request_file} (worktree={worktree})"
        )
        return
    ...
    process.stdin.write(prompt(root, state, phase, args.light))
    process.stdin.close()
    phase.update(
        {
            "status": "RUNNING",
...
def poll(root: Path, state: dict, args: argparse.Namespace) -> None:
    for phase in state["phases"].values():
        ...
        phase["pid"] = None
    for phase in state["phases"].values():
        if not phase.get("branch") or phase["status"] not in {
            "READY_TO_COMMIT",
            "PR_OPEN",
...
def main(argv: list[str], light: bool) -> int:
    ...
    parser.add_argument(
        "command", nargs="?", choices=["next", "status", "review", "approve", "switch"]
    )
