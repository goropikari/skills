...
STEPS = [
    ...
    },
    {
        ...
        "description": "テスト分析・設計と関数定義に従い、対象プロジェクトのテスト構成に合う場所へ自動テストを実装する。",
    },
    {
        "index": 5,
        "name": "6. 内部ロジック実装",
...
def print_agent_instruction(step_num, script_path=None, continuing=False):
    ...
    if step_num == 0:
        ...
        print(
            f"3. 合意後、ターゲットファイル `{CYAN}{step['file']}{RESET}` に要件定義書を{action}してください。"
        )
        print(
        ...
        if script_path:
            print(
                f"6. 成果物の出力後、AI自身でコマンド `{CYAN}python3 {script_path} review{RESET}` を実行してレビュー待ち状態にしてください。"
            )
        return

    if step_num == 1:
        print(
            f"1. 入力元 `{CYAN}.dev-workflow/01_requirements.md{RESET}` を必ず読み、要件定義書からGherkin定義を作成してください。"
        )
        print(
            f"2. ターゲットファイル `{CYAN}{step['file']}{RESET}` に、Feature、Scenario、Given/When/Then 形式で期待される振る舞いを記述してください。"
        )
        print(
            "3. ユーザーへ追加質問せず、判断できない内容はコメントまたは未決事項として明記してください。"
        )
        print(
            "4. 今何が終わっていて、次に何が残っているのかのサマリ（完了したこと、残タスク）を"
        )
    ...
    if step_num == 2:
        print(
            f"1. 入力元 `{CYAN}.dev-workflow/01_requirements.md{RESET}` と `{CYAN}.dev-workflow/02_features/spec.feature{RESET}` を必ず読み、テスト分析・設計を自動生成してください。"
        )
        print(
            f"2. ターゲットファイル `{CYAN}{step['file']}{RESET}` に、テスト観点、テスト条件、正常系・異常系・境界値、リスク、受け入れ条件との対応を記述してください。"
        )
        print(
            "3. ユーザーへ追加質問せず、判断できない内容は未決事項またはテストリスクとして明記してください。"
        )
        print(
            "4. 今何が終わっていて、次に何が残っているのかのサマリ（完了したこと、残タスク）を"
        )
    ...
    if step_num >= 3:
        print(
            f"1. ターゲット範囲 `{CYAN}{step['file']}{RESET}` で、対象プロジェクトの既存構成に合う場所を選んで実装してください。"
        )
    else:
        print(
            f"1. ターゲットファイル `{CYAN}{step['file']}{RESET}` に対する実装・作成作業を進めてください。"
        )
    print(
        "2. 今何が終わっていて、次に何が残っているのかのサマリ（完了したこと、残タスク）を"
    )
    print(
        "   `.dev-workflow/CURRENT_STEP.md` の「進捗サマリ」セクションに随時記録・更新してください。"
    )
    print(
        "3. ステップの作業が完了したら、生成・更新した成果物のパスと内容または要約をユーザーに出力してください。"
    )
    if script_path:
        print(
            f"4. 成果物の出力後、AI自身でコマンド `{CYAN}python3 {script_path} review{RESET}` を実行してレビュー待ち状態にしてください。"
        )
        ...
        try:
            ...
            if step_match and int(step_match.group(1)) == step_num:
                ...
                updated_content = re.sub(
                    r"-\s+\*\*Status\*\*:\s*[A-Z_]+", f"- **Status**: {status}", content
                )
...
def main():
    ...
    is_review = len(sys.argv) > 1 and sys.argv[1] in (
        "review",
        "submit",
        "review-pending",
        "review_pending",
    ...
    if is_next:
        if status == "IN_PROGRESS":
            ...
            print(
                f"{YELLOW}{BOLD}⚠️  DEV-WORKFLOW: CURRENT STEP IN PROGRESS / 作業継続指示{RESET}"
            )
            print(f"{YELLOW}{BOLD}========================================={RESET}")
        ...
        if status == "REVIEW_PENDING":
            print(f"{CYAN}{BOLD}========================================={RESET}")
            print(
                f"{CYAN}{BOLD}⏳  DEV-WORKFLOW: REVIEW PENDING / レビュー確認待ち{RESET}"
            )
            print(f"{CYAN}{BOLD}========================================={RESET}")
            ...
            print(
                "1. 人間（ユーザー）からのレビュー確認または「OK」「進めて」などの指示を待ってください。"
            )
            print(
                "2. 承認指示を受け取ったら、AI自身でステータスを承認済みに更新してください："
            )
            print(
                f"   - コマンド `{CYAN}python3 {script_path} approve{RESET}` を実行する"
            )
            print(
                f"3. 承認更新後、再度 `{CYAN}{DW_NEXT_COMMAND}{RESET}` コマンドを実行して次のステップへ進んでください。"
            )
            print(f"{CYAN}{BOLD}========================================={RESET}")
            sys.exit(0)
        ...
        if step_num >= len(STEPS) - 1:
            ...
            print(f"{CYAN}{BOLD}========================================={RESET}")
            print(
                f"{CYAN}{BOLD}➡️  DEV-WORKFLOW STEP TRANSITION (Step {old_step + 1}/{len(STEPS)} -> {new_step + 1}/{len(STEPS)}){RESET}"
            )
            print(f"{CYAN}{BOLD}========================================={RESET}")
        ...
        print(
            f"{RED}次へ進む指示があるまで、絶対にこれ以降のステップのファイルを生成・変更しないでください。{RESET}"
        )
        print(f"{BLUE}{BOLD}========================================={RESET}")
