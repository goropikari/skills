...
name: dw-phase
description: >-
  ユーザーが Codex で $dw-phase / $dw-phase next / $dw-phase review / $dw-phase approve、
  または Claude Code で /dw-phase / /dw-phase next / /dw-phase review / /dw-phase approve を入力したときに
...
Codex と Claude Code のどちらで実行しても、同じ対象プロジェクトの作業ディレクトリで実行する限り、同じ `.dev-workflow-phase/` の状態を共有します。Codex では `$dw-phase`、Claude Code では `/dw-phase` を同じ操作の別名として扱ってください。

1. `$dw-phase` / `/dw-phase`
   - 実行するコマンド: `python3 "<このスキルディレクトリ>/scripts/workflow.py"`

2. `$dw-phase next` / `/dw-phase next`
   - 実行するコマンド: `python3 "<このスキルディレクトリ>/scripts/workflow.py" next`

3. `$dw-phase review` / `/dw-phase review`
   - 実行するコマンド: `python3 "<このスキルディレクトリ>/scripts/workflow.py" review`

4. `$dw-phase approve` / `/dw-phase approve`
